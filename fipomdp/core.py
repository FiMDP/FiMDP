...
class BeliefSuppConsMDP(ConsMDP):
    ...
    def new_state(self, belief_support: List[int], reload: bool = False, name: str = None) -> None:
...
class GuessingConsMDP(BeliefSuppConsMDP):
    ...
    def new_state(self, belief_support: List[int], reload: bool = False, name: str = None,  guess: Optional[int] = None) -> None:
...
class ConsPOMDP(ConsMDP):
    ...
    def set_observations(self, num_observations: int, obs_probs: Dict[Tuple[int, int], float],
                         obs_names: Optional[List[str]] = None) -> None:
    ...
    def compute_belief_supp_cmdp_initial_state(self, initial_belief_supp: List[int]) -> None:
        ...
        if len(initial_belief_supp) == 0:
            raise AttributeError(f"Initial belief support {initial_belief_supp} should not be empty!")
        ...
        belief_supp_cmdp.new_state(initial_belief_supp, self.reloads[initial_belief_supp[0]], name)
        while len(queue) > 0:
    ...
    def _add_belief_supp_action_with_label(self, belief_src: List[int], label: str, bel_supp_actions_same_label: List[ActionData],
        ...
        for obs_distr, same_obs_distrs_groups in groupby(act_distrs, lambda distr: self.get_state_obs_probs(distr[0])):
    ...
    def compute_guessing_cmdp_initial_state(self, initial_belief: List[int], initial_guess: int) -> None:
        ...
        guessing_cmpd.new_state(initial_belief, self.reloads[initial_belief[0]], name, initial_guess)
